...
#	if previousColumn + 1 >= len(numbers[line]): rightChild = 0
#	else: rightChild = numbers[line][previousColumn+1]
#	
#	leftChild = numbers[line][previousColumn]
#	maxChild = max([leftChild, rightChild])
#	print(maxChild)
#	theSum += maxChild
#	if rightChild == maxChild: previousColumn += 1
#
#print(theSum)

##stackOverflow freaking rocks
#def all_paths(tree, r, c):
#	current = tree[r][c]
#	if r < len(tree) - 1:
#		below_paths = all_paths(tree, r+1, c) + all_paths(tree, r+1, c+1)
#		shit = [[current] + path for path in below_paths]
#		return shit
#	else:
#		return [[current]]
#	
#print(max([sum(i) for i in all_paths(numbers, 0, 0)]))
